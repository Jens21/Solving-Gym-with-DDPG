...
def sample_action(obs, net_policy, itt):
    threshold = max(0.1, 1 - itt / 30_000)
    obs = obs[None]
...
def main():
    ...
    actor_policy, actor_target = Actor(input_size=4, action_size=1), Actor(input_size=4, action_size=1)
    actor_target.load_state_dict(actor_policy.state_dict())
    critic_policy, critic_target = Critic(input_size=4, action_size=1), Critic(input_size=4, action_size=1)
    ...
    for itt in range(N_TOTAL):
        ...
        reward = 0
        for _ in range(5):
            next_obs, r, done, _, _ = env.step(action * 3)
            reward += r
            if done:
                break
