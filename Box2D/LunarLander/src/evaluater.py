...
class Evaluater():
    ...
    def get_network_action(self, state):
        ...
        out, out2 = self.network.actor_policy(state)

        if state[0, 6] or state[0, 7]:
            out[:] = 0

        return out.cpu().detach().numpy()[0], out2.cpu().detach().numpy()[0]

    def get_action(self, state, itt):
        # if itt < 50_000:
        #     std = min(0.1, 1 - itt/100_000)
        noise = np.random.normal(0, 0.6, self.action_size)
        ...
        # else:
        action, state = self.get_network_action(state)
        action += noise
